...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def words(word):
        word=split(lower(remove_punctuation(word)))
        for i in subject:
            for m in word:
                if m == i:
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if typed_word == i:
            return typed_word
    all_values=[diff_function(typed_word,i,limit) for i in word_list]
    min_value=min(all_values)
    if min_value>limit:
        return typed_word
    location=all_values.index(min_value)
...
def furry_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    #base case
    def tail_recursion(temp,temp_goal,limitd,count):
        if temp == "" or temp_goal == "":
            return count
        if temp[0] !=temp_goal[0]:
            count+=1
        if count > limitd:
            return count+1
        return tail_recursion(temp[1:],temp_goal[1:],limitd,count)
    return tail_recursion(typed,source,limit,count=abs(len(source)-len(typed)))   
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    if typed==[]:
        if source!=[]:
            ...
            return 0.0
        upload({'id':user_id,'progress':100.0})
        return 100.0
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    y=[]
    for m in range(len(timestamps_per_player)):
        x=[abs(timestamps_per_player[m][n]-timestamps_per_player[m][n+1]) for n in range(len(timestamps_per_player[m])-1)] 
        y+=[x]
    return words,y
    # END PROBLEM 9
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    x,y=time_per_word(words,timestamps_per_player)
    return match(x,y)
...
def fastest_words(match_object):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    lst=[[] for i in player_indices]
    time_lst=[[] for i in word_indices ]
    for m in word_indices:
        time_lst[m]+=[get_time(match_object,n,m) for n in player_indices]
    for p in word_indices:
        lst[time_lst[p].index(min(time_lst[p]))]+=[get_word(match_object,p)]
    return lst
    # END PROBLEM 11

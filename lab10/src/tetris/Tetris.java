...
public class Tetris {
    ...
    private void incrementScore(int linesCleared) {
        ...
        scores.put(3, 500);
        scores.put(4, 800);
        if(scores.containsKey(linesCleared)) {
            score += scores.get(linesCleared);
        }
    }
    ...
    /**
     * ShiftDown current row and more height
     * @param col
     */
    private void shiftDown(TETile[][] tiles, int col){
        for(; col < tiles[0].length; col++){
